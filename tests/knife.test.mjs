...
const {
  ...
  resolveKnifeAttack,
  traceKnife,
  KNIFE_SWING_DISTANCE,
  KNIFE_STAB_DISTANCE,
  KNIFE_ARMOR_RATIO
} = require(join(output, 'knife-rules.js'))
...
test('knife primary uses the original paused and chained damage with hit-dependent cooldowns', () => {
  ...
  near(state.nextPrimary, 100.4)
  near(state.nextSecondary, 100.5)
  ...
  assert.equal(chained.damage, 15)
  near(state.nextPrimary, 100.8)
  ...
  const miss = freshKnifeCooldown()
  resolveKnifeAttack(miss, 'swing', 200, false)
  near(miss.nextPrimary, 200.35)
  near(miss.nextSecondary, 200.5)
})

test('knife stab uses 65 damage, triples a backstab, and shares its hit or miss lockout', () => {
  const front = freshKnifeCooldown(),
    back = freshKnifeCooldown(),
    miss = freshKnifeCooldown()
  assert.equal(resolveKnifeAttack(front, 'stab', 300, true, 'body', false).damage, 65)
  near(front.nextPrimary, 301.1)
  near(front.nextSecondary, 301.1)
  ...
  near(miss.nextPrimary, 301)
  near(miss.nextSecondary, 301)
})
...
test('knife backstab requires attacker and victim horizontal facing within the source dot threshold', () => {
  ...
  assert.equal(isKnifeBackstab({ x: 0.7, y: 0, z: 0.7 }, { x: 0, y: 0, z: 1 }), false)
...
test('knife reach distinguishes swing and stab, respects walls, and uses the original armor ratio', () => {
  near(KNIFE_SWING_DISTANCE, 1.2)
  near(KNIFE_STAB_DISTANCE, 0.8)
  near(KNIFE_ARMOR_RATIO, 0.85)
  const origin = { x: 0, y: 1.6, z: 0 },
    direction = { x: 0, y: 0, z: 1 }
  const target = {
    id: 'enemy',
    center: { x: 0, y: 0, z: 1.4 },
    regions: [{ group: 'body', y: 1.6, half: { x: 0.05, y: 0.05, z: 0.05 } }]
  }
  ...
  const blocked = traceKnife(origin, direction, [target], 'swing', () => 0.5)
  assert.equal(blocked.hit, undefined)
  assert.equal(blocked.contact, true)
  near(blocked.distance, 0.5)
  const armored = armorDamage(65, 100, true, 'body', false, KNIFE_ARMOR_RATIO)
  near(armored.damage, 55.25)
  near(armored.armor, 95.125)
})
