...
execFileSync(process.execPath, ['node_modules/typescript/bin/tsc', 'src/bot-objective.ts', 'src/bot-navigation.ts', 'src/bomb-rules.ts', '--target', 'es2020', '--module', 'commonjs', '--outDir', output, '--skipLibCheck', '--resolveJsonModule', '--esModuleInterop'])
...
const bomb = overrides => ({ phase: 'carried', carrier: botAddress(0), planter: '', defuser: '', site: '', position: { x: 0, y: 0, z: 0 }, actionStarted: 0, actionEnds: 0, explodeAt: 0, progress: 0, round: 1, ...overrides })
...
test('the closest living bot retrieves a dropped bomb with stable tie breaking', () => {
  ...
  const objectives = soloBombObjectives(candidates, bomb({ phase: 'dropped', carrier: '', position: { x: 3, y: 0, z: 0 } }), 1)
...
test('living bots spread around the active planted site and resolved bombs clear objectives', () => {
  const active = soloBombObjectives(bots.map(bot => ({ ...bot, alive: bot.index !== 1 })), bomb({ phase: 'planted', carrier: '', position: soloBombSite(1) }), 1)
  ...
  for (const phase of ['defused', 'exploded', 'none']) assert.equal(soloBombObjectives(bots, bomb({ phase, carrier: '' }), 1).size, 0)
})

test('a carrier follows the production route and completes an uninterrupted solo plant', () => {
  const address = botAddress(0)
  const navigation = createBotNavigation({ x: 88, y: 8.0546, z: 50 }, 0)
  const state = freshBomb(1, address, navigation.position)
  let event
  for (let step = 0; step < 200 && event !== 'planted'; step++) {
    const now = step / 10
    const objective = soloBombObjectives([{ index: 0, alive: true, position: navigation.position }], state, 1).get(address)
    moveBot(navigation, undefined, now, 0.1, 5.525, undefined, undefined, objective)
    const atSite = Math.hypot(navigation.position.x - soloBombSite(1).x, navigation.position.z - soloBombSite(1).z) < 0.8
    event = stepBomb(state, [{ address, team: 1, alive: true, position: navigation.position, grounded: true, holding: atSite, selected: atSite, canDefuse: false, hasKit: false }], now, true, position => Math.hypot(position.x - soloBombSite(1).x, position.z - soloBombSite(1).z) < 0.8 ? 'B' : '')
  }
