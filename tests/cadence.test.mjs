...
const rate = .0955
const fresh = () => ({ ammoClip:30,maxAmmoClip:30,ammoReserve:90,fireRate:rate,lastShotTime:0,lastShotId:0,isReloading:false,reloadTime:2.45,reloadStartTime:0 })
...
test('a request arriving 20 ms early waits without spending ammo or firing before the deadline', () => {
 ...
 const at=shotDeadline(weapon.lastShotTime,rate,epoch+rate-.02)
 assert.equal(at,epoch+rate)
 assert.equal(fireShot(weapon,at-.001,true,at),'cooldown')
...
test('thirty full-auto shots survive 60 Hz client frames, 30 Hz server ticks, and variable arrival delay', () => {
 ...
 for(let frame=0;requests.length<30;frame++) {
  ...
  requests.push({id:requests.length+1,at:now+.04+[0,.008,-.008,.012,-.012][requests.length%5]})
 }
 const weapon=fresh(),legacy=fresh(),fired=[]
 let pending, index=0,legacyCount=0
...
test('requests far ahead of cadence cannot queue; a long stall starts a fresh schedule', () => {
 assert.equal(shotDeadline(epoch,.4,epoch+.2),undefined)
...
test('deferred shots still honor death, reload, and ammo at execution time', () => {
 for(const [overrides,alive,reason] of [
  ...
  claimShot(weapon,1)
  assert.equal(fireShot(weapon,epoch+.1,alive,epoch+.0955),reason)
  assert.equal(weapon.ammoClip,ammo)
 }
 const weapon={...fresh(),isReloading:true,ammoClip:0,reloadStartTime:epoch-2.35}
 assert.equal(fireShot(weapon,epoch+.11,true,epoch+.0955),undefined,'reload uses actual execution time')
 assert.equal(weapon.ammoClip,29)
})
