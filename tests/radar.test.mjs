...
const origin = { x: 0, y: 0, z: 0 },
  forward = { x: 0, y: 0, z: 1 }
const player = (address, overrides = {}) => ({
  address,
  team: 1,
  connected: true,
  alive: true,
...
test('height glyph switches at the source 128-unit boundary', () => {
  ...
  assert.equal(at((128 * 2) / 75 - 0.001), 'dot')
...
test('radar excludes self, enemies, dead, disconnected and waiting players', () => {
  const players = [
    ...
    player('friend'),
    player('enemy', { team: 2 }),
    player('dead', { alive: false }),
    player('left', { connected: false }),
    player('queued', { eligibleRound: 2 })
  ]
  assert.deepEqual(
    radarMarkers(players, 'self', 1, 1, origin, forward).map((m) => m.id),
    ['friend']
  )
...
test('C4 carrier is red and ground/planted bomb markers are Terrorist-only and flash', () => {
  ...
  for (const phase of ['dropped', 'planted']) {
    ...
    assert.equal(t[0].shape, phase === 'planted' ? 'bomb' : 'dot')
  ...
  for (const phase of ['none', 'defused', 'exploded'])
    assert.equal(radarMarkers([], 'self', 1, 1, origin, forward, { ...bomb, phase }).length, 0)
})
