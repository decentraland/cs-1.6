...
execFileSync(process.execPath,['node_modules/typescript/bin/tsc','src/inventory-rules.ts','src/gun-accuracy.ts','src/recoil-prediction.ts','--target','es2020','--module','commonjs','--outDir',output,'--skipLibCheck'])
const { startingInventory,buyGun,buyGunAmmo,selectWeapon,SemiAutoTrigger }=require(join(output,'inventory-rules.js'))
...
test('rifle buying enforces side and funds, charges once, and includes only its magazine',()=>{
 const inventory=startingInventory(2),before=structuredClone(inventory)
 ...
 assert.equal(inventory.active,'m4a1');assert.deepEqual(inventory.items[1],{id:'m4a1',clip:30,reserve:0})
...
test('switching preserves both magazines and reserves, and cannot select an absent slot',()=>{
 ...
 inventory.items[0].clip=13;inventory.items[1].clip=17
 ...
 assert.deepEqual(inventory.items.map(i=>[i.clip,i.reserve]),[[13,40],[17,0]])
})
test('ammo purchases use each caliber price, pack and capacity independently of selected slot',()=>{
 const inventory=startingInventory(2);buyGun(inventory,'m4a1',2,4000)
 ...
 inventory.items[0].reserve=99;buyGunAmmo(inventory,'secondary',100)
...
test('pistol purchase replaces only the secondary slot and preserves the rifle',()=>{
 ...
 assert.deepEqual(inventory.items.map(i=>i.id),['m4a1','glock18'])
})
test('one pistol shot per press also works when a short tap is released before the shot arrives',()=>{
 ...
 trigger.update(true);assert.equal(trigger.claim(),true)
 for(let i=0;i<20;i++){trigger.update(true);assert.equal(trigger.claim(),false)}
 trigger.update(false);trigger.update(true);trigger.update(false)
 assert.equal(trigger.claim(),true);assert.equal(trigger.claim(),false)
 trigger.reset();assert.equal(trigger.claim(),false)
})
test('USP and Glock use previous-shot accuracy, rapid-tap penalties and their own camera punch',()=>{
 for(const id of ['usp','glock18']) {
  const state=freshGunAccuracy(id),base=GUNS[id].accuracy
  near(gunSpread(state,id,100,0,true),.1*(1-base))
  gunKick(state,id,0,true,()=>.5)
  near(state.pitch,id==='usp'?2:0);near(state.yaw,0)
  near(gunSpread(state,id,100.15,0,true),.1*(1-base))
  assert.ok(state.accuracy<base)
  const previous=state.accuracy
  near(gunSpread(state,id,101,0,true),.1*(1-previous));near(state.accuracy,base)
  assert.ok(gunSpread(state,id,102,6.25,true)>.1*(1-base))
  assert.ok(gunSpread(state,id,103,0,false)>.1*(1-base)*5)
 }
})
test('M4A1 accuracy uses floating point division and its own moving recoil limits',()=>{
 const state=freshGunAccuracy('m4a1')
 near(gunSpread(state,'m4a1',100,0,true),.004)
 near(state.accuracy,1/220+.3)
 gunKick(state,'m4a1',0,true,()=>.5);near(state.pitch,.65)
 for(let i=0;i<30;i++){gunSpread(state,'m4a1',100.01+i*.01,5.75,true);gunKick(state,'m4a1',5.75,true,()=>.5)}
 assert.equal(state.accuracy,1);assert.ok(state.pitch<=3.75);assert.ok(Math.abs(state.yaw)<=3)
})
test('local pistol and M4 prediction matches server punch without replaying delayed acknowledgements',()=>{
 for(const id of ['usp','glock18','m4a1']) {
  const recoil=new RecoilPrediction(id),server=freshGunAccuracy(id)
  recoil.trigger(true,100);recoil.predict(1,100,0,true)
  gunSpread(server,id,100,0,true);gunKick(server,id,0,true,()=>.5)
  near(recoil.punch(100).pitch,server.pitch)
  const before=recoil.punch(100.4);recoil.confirm(1,server,100.4)
