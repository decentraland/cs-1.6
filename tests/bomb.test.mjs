...
test('planting requires a live carrier, bomb selected, ground, site, and three uninterrupted seconds', () => {
  ...
  assert.equal(
    stepBomb(state, [t()], 12.999, true, () => 'A'),
    undefined
  )
  assert.equal(
    stepBomb(state, [t()], 13, true, () => 'A'),
  ...
  assert.equal(state.explodeAt, 58)
  assert.equal(state.carrier, '')
  assert.equal(state.planter, 't')
})

test('release, movement, weapon switch, leaving site, and jumping cancel planting progress', () => {
  for (const [player, site] of [
    [t({ holding: false }), 'A'],
    [t({ selected: false }), 'A'],
    [t({ grounded: false }), 'A'],
    [t({ position: { x: 0.5, y: 0, z: 0 } }), 'A'],
    [t(), 'B']
  ]) {
    const state = freshBomb(1, 't')
    stepBomb(state, [t()], 10, true, () => 'A')
    stepBomb(state, [player], 12.9, true, () => site)
    assert.equal(state.phase, 'carried')
...
test('carrier death drops the C4; only living Terrorists can pick it up', () => {
  const state = freshBomb(1, 't')
  stepBomb(state, [t({ alive: false }), ct()], 10, true, () => 'A')
...
test('defuse duration is ten seconds without a kit and five with a kit', () => {
  for (const [hasKit, duration] of [
    [false, 10],
    [true, 5]
  ]) {
    const state = planted()
    stepBomb(state, [ct({ hasKit })], 20, true, () => 'A')
    assert.equal(state.actionEnds, 20 + duration)
    assert.equal(
      stepBomb(state, [ct({ hasKit })], 20 + duration - 0.001, true, () => 'A'),
      undefined
    )
    assert.equal(
      stepBomb(state, [ct({ hasKit })], 20 + duration, true, () => 'A'),
      'defused'
    )
    assert.equal(
      stepBomb(state, [ct({ hasKit })], 70, true, () => 'A'),
      undefined,
      'resolved bombs cannot explode later'
    )
  }
})
...
test('defuse release, range loss, jumping, and death erase progress; another CT cannot steal an active defuse', () => {
  for (const player of [
    ...
    const state = planted()
    stepBomb(state, [ct()], 20, true, () => 'A')
    stepBomb(state, [player], 25, true, () => 'A')
    assert.equal(state.defuser, '')
    assert.equal(state.progress, 0)
    stepBomb(state, [ct()], 26, true, () => 'A')
    assert.equal(state.actionEnds, 36)
  }
  const state = planted()
  stepBomb(state, [ct()], 20, true, () => 'A')
  stepBomb(state, [ct({ address: 'other', hasKit: true }), ct()], 23, true, () => 'A')
  assert.equal(state.defuser, 'ct')
  assert.equal(state.actionEnds, 30)
})

test('the fuse wins ties; an earlier completed defuse survives a delayed server tick', () => {
  const tied = planted()
  stepBomb(tied, [ct()], 48, true, () => 'A')
  assert.equal(
    stepBomb(tied, [ct()], 58, true, () => 'A'),
    'exploded'
  )
  const earlier = planted()
  stepBomb(earlier, [ct()], 47.9, true, () => 'A')
  assert.equal(
    stepBomb(earlier, [ct()], 58.1, true, () => 'A'),
    'defused'
  )
  const late = planted()
  assert.equal(
    stepBomb(late, [ct()], 58, true, () => 'A'),
    'exploded'
  )
})

test('C4 awards three frags only to the defuser or successful explosion planter', () => {
  const defused = planted()
  stepBomb(defused, [ct()], 20, true, () => 'A')
  const defuseEvent = stepBomb(defused, [ct()], 30, true, () => 'A')
  assert.deepEqual(bombFragAward(defused, defuseEvent), { address: 'ct', frags: 3 })
  const exploded = planted(),
    explosionEvent = stepBomb(exploded, [], 58, true, () => 'A')
...
test('plant overrides the round clock and T elimination; explosion and defuse award the proper side', () => {
  const state = {
    ...
    matchOver: false,
    roster: [
      { address: 'ct', team: 2, eligibleRound: 1, connected: true },
      { address: 't', team: 1, eligibleRound: 1, connected: true }
    ]
  }
  assert.equal(
    teamRoundWinner(state, (address) => address === 'ct', 0, 'planted'),
    undefined
  )
  assert.equal(
    teamRoundWinner(state, (address) => address === 't', 50, 'planted'),
    't'
  )
  assert.equal(
    teamRoundWinner(state, () => true, 50, 'defused'),
    'ct'
  )
  assert.equal(
    teamRoundWinner(state, () => true, 50, 'exploded'),
    't'
  )
  assert.equal(
    teamRoundWinner(state, (address) => address === 'ct', 50, 'carried'),
    'ct'
  )
})
...
test('C4 switches through all five reference beep waves during the fuse', () => {
  assert.equal(bombBeepWave(0), 1)
  assert.equal(bombBeepWave(10.99), 1)
  assert.equal(bombBeepWave(11), 2)
