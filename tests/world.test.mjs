...
execFileSync(process.execPath, ['node_modules/typescript/bin/tsc', 'src/world-query.ts', 'src/map-collision.ts', 'src/team-spawns.ts', 'src/bomb-sites.ts', '--target', 'es2020', '--module', 'commonjs', '--outDir', output, '--skipLibCheck'])
...
test('all three encounter positions have unobstructed sight from spawn', () => {
  for (const [x, z] of [[88, 50], [85, 54], [90, 48]]) {
...
test('bot hitboxes reject misses and rays behind the shooter', () => {
  const center = { x: 10, y: 1, z: 0 }, size = { x: 0.4, y: 0.9, z: 0.35 }
...
test('all original team spawns have floor support, standing clearance, and distinct positions', () => {
  for (const team of [1, 2]) {
    ...
    for (let slot = 0; slot < 20; slot++) {
      ...
      assert.ok(Math.abs(down - .08) < .002)
      for (const [x, z] of [[0, 0], [.25, 0], [-.25, 0], [0, .25], [0, -.25]]) {
        assert.equal(mapDistance({ x: position.x + x, y: position.y, z: position.z + z }, { x: 0, y: 1, z: 0 }, 1.85), 1.85)
      }
      assert.ok(positions.every(other => Math.hypot(other.x-position.x, other.z-position.z) > .6))
...
test('both original Dust2 bomb zones contain a walkable planting surface', () => {
  for (const zone of BOMB_SITES) {
    ...
    assert.ok(floor<10)
    const feet={...origin,y:origin.y-floor+.08}
...
test('original Dust2 buy volumes contain all team spawns and reject enemy spawns and bomb sites',()=>{
 for(const team of [1,2])for(let i=0;i<20;i++) {
 ...
 for(const position of [{x:32.8375,y:12.586,z:46.7017},{x:104.5175,y:10.026,z:40.7284}]) {
  assert.equal(inBuyZone(position,1),false);assert.equal(inBuyZone(position,2),false)
 }
})
