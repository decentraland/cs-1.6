...
async function kill(victim) {
  ...
  for (let i = 0; i < 5; i++) {
    await enemy.shoot(70); await pause(400)
...
try {
  ...
  await observer.until(state => labels(state).includes('Select a team'), 'fresh team menu', 50000)
  await observer.join(2, true)
  for (const friend of [first, second]) {
    await friend.until(state => labels(state).includes('Select a team'), 'team lobby')
    await friend.join(2, false)
    await friend.until(state => labels(state).includes('Joined Counter-Terrorists'), 'teammate admitted')
  }
  await enemy.until(state => labels(state).includes('Select a team'), 'enemy lobby')
  await enemy.join(1, false)
  await observer.until(state => readHud(state)?.health === 100 && readHud(state)?.seconds > 100 && !labels(state).includes('Prepare to fight!'), 'live round')
  ...
  await kill(observer)
  const corpse = position(await observer.snapshot()), firstEye = eye(position(await first.snapshot())), secondEye = eye(position(await second.snapshot()))
  await observer.until(state => spectating(state) && distance(camera(state), firstEye) < 3.2, 'camera follows first living teammate')
  ...
  await pause(70); await observer.shoot(70)
  ...
  await observer.key('keyDown', 'w', 'KeyW', 87); await pause(500); await observer.key('keyUp', 'w', 'KeyW', 87)
  const frozen = await observer.snapshot()
  assert.ok(distance(position(frozen), corpse) < .15, 'spectating cannot move the corpse')
  ...
  await first.command('/move_player_to 90 14 67'); await pause(1000)
  const movedEye = eye(position(await first.snapshot()))
  await observer.until(state => distance(camera(state), movedEye) < 3.2 && distance(camera(state), previousCamera) > .5, 'camera follows moving teammate')
  await kill(first)
  await observer.until(state => spectating(state) && distance(camera(state), secondEye) < 3.2, 'target death automatically selects survivor')
  ...
  await observer.until(state => labels(state).includes('Prepare to fight!') && readHud(state)?.health === 100, 'respawn next round', 8000)
  const respawn = await observer.until(state => !spectating(state) && distance(camera(state), eye(position(state))) < .25, 'respawn restores own first-person camera')
  assert.equal(readHud(respawn).clip, 12)
  await observer.until(state => !labels(state).includes('Prepare to fight!') && readHud(state)?.seconds > 100, 'next live round')
  await observer.capture(); await observer.shoot(70)
