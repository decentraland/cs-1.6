...
import { Client,pause,labels,readHud,readTeamScore } from './team-client.mjs'
...
const hasHumanScore=(state,score,deaths)=>readScoreRows(state).some(row=>row.values.at(-3)===String(score)&&row.values.at(-2)===String(deaths)&&row.values.at(-1)==='-')
async function key(client,key,code,vk){await client.key('keyDown',key,code,vk);await pause(70);await client.key('keyUp',key,code,vk);await pause(200)}
async function plant(position, cancel=false) {
 const initialClip=readHud(await t.snapshot()).clip
 await t.command(`/move_player_to ${position.x} ${position.y} ${position.z}`);await pause(1200)
 await t.capture();await key(t,'4','Digit4',52)
 await t.until(s=>labels(s).some(v=>v.startsWith('C4 —')),'C4 selected')
 if(cancel){
  await t.shoot(800);await pause(450)
 ...
 const bomb={x:foot.x,y:foot.y+.08,z:foot.z}
...
try {
 ...
 await ct.until(s=>labels(s).includes('Select a team'),'fresh team menu',45000)
 await ct.join(2,true);await ct.until(s=>labels(s).includes('Joined Counter-Terrorists'),'CT joined')
 ...
 await key(t,'3','Digit3',51)
 await t.command(`/move_player_to ${original.x+2} ${original.y+.1} ${original.z}`)
 await pause(1000)
 assert.ok(!labels(await t.snapshot()).some(v=>v.startsWith('C4')),'dropping removes carrier status')
 await t.command(`/move_player_to ${original.x} ${original.y+.1} ${original.z}`)
 ...
 await t.command('/move_player_to 109.4 11 35.8');await pause(1200)
 await key(t,'4','Digit4',52);await t.shoot(3600);await pause(300)
 ...
 await ct.command(`/move_player_to ${a.x+3} ${a.y+.1} ${a.z}`);await pause(1200)
 await ct.capture();const feet=(await t.snapshot())['1'].Transform.position
 await ct.aimAt({x:feet.x,y:feet.y+1.6,z:feet.z})
 for(let i=0;i<5;i++){await ct.shoot();await pause(400);if(readHud(await t.snapshot()).health===0)break}
 await t.until(s=>readHud(s)?.health===0,'planter dead')
 await pause(1000)
 assert.ok(!labels(await ct.snapshot()).includes('Counter-Terrorists Win!'),'killing last T does not beat planted bomb')
 evidence.checks.push('plant survives elimination of last Terrorist')
 await ct.command(`/move_player_to ${a.x+1.3} ${a.y+.1} ${a.z}`);await pause(800)
 await ct.aimAt(a)
 await ct.key('keyDown','e','KeyE',69)
 await ct.until(s=>labels(s).includes('Defusing the bomb'),'defuse begins')
 await pause(800);await ct.key('keyUp','e','KeyE',69);await pause(600)
 assert.ok(!labels(await ct.snapshot()).includes('Defusing the bomb'),'release cancels defuse')
 const began=Date.now();await ct.key('keyDown','e','KeyE',69)
 await ct.until(s=>labels(s).includes('The bomb has been defused!'),'ten-second defuse',13000)
 await ct.key('keyUp','e','KeyE',69)
 evidence.defuseMs=Date.now()-began;assert.ok(evidence.defuseMs>=9900&&evidence.defuseMs<12500)
 const defuseBoard=await t.scoreboard()
 assert.equal(readTeamScore(defuseBoard,2),1,'shared defuse score')
 assert.ok(hasHumanScore(defuseBoard,4,0),'defuser receives three frags in addition to the planter kill')
 ...
 await ct.command(`/move_player_to ${b.x+.5} ${b.y+.1} ${b.z}`)
 await t.command('/move_player_to 85 14 131')
 const fused=Date.now()
 const beepWaves=new Set()
 const exploded=await ct.until(s=>{
   for(const c of Object.values(s))if(c.AudioSource?.audioClipUrl?.includes('c4_beep'))beepWaves.add(c.AudioSource.audioClipUrl)
   return labels(s).includes('Target successfully bombed!')
 },'45-second fuse',49000)
 ...
 assert.equal(readTeamScore(explosionBoard,1),1,'shared bomb explosion score')
