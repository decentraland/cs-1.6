...
try {
 ...
 await pause(4000)
 await stateUntil(s=>labels(s).includes('Start game')||labels(s).includes('Play again'),'start menu',25000)
 ...
 await stateUntil(s=>labels(s).includes('Prepare to fight!'),'freeze',15000)
 ...
 for(let i=0;i<12;i++) {
  await pause(70)
  const state=await snapshot()
  samples.push({...angles(state['2'].Transform.rotation),hud:readHud(state),labels:labels(state),locked:state['2'].PointerLock?.isPointerLocked})
 }
 await send('Input.dispatchMouseEvent',{type:'mouseReleased',x:mx,y:my,button:'left',clickCount:1})
 await pause(1000)
 const settled=await snapshot()
 const final=angles(settled['2'].Transform.rotation)
 const rise=Math.max(...samples.map(s=>s.pitch-base.pitch))
 console.log('RECOIL',JSON.stringify({rise,final}))
 assert.ok(rise>0.015,'sustained fire visibly raises camera')
 assert.ok(Math.abs(final.pitch-base.pitch)<0.01,'camera settles back to mouse aim')
 mx+=Math.PI/4/0.005236
 await send('Input.dispatchMouseEvent',{type:'mouseMoved',x:mx,y:my})
 await pause(200)
 const rotated=await snapshot()
 const view=angles(rotated['2'].Transform.rotation)
 assert.ok(Math.abs(wrap(view.yaw-base.yaw)-Math.PI/4)<.08,'mouse controls camera yaw')
 ...
 assert.ok(distance>.6,'W moves player')
 assert.ok(alignment>.9,'W follows the rotated camera heading')
 const evidence={date:new Date().toISOString(),input:'left mouse',riseRadians:rise,settledPitch:final.pitch,basePitch:base.pitch,mouseYawRadians:wrap(view.yaw-base.yaw),movement:{distance,alignment},samples}
 if(process.argv[3]) await writeFile(process.argv[3],JSON.stringify(evidence,null,2)+'\n')
 console.log('PASS: visible recoil → recovery → mouse turn → movement follows camera')
} finally {
 if(sessionId) {
