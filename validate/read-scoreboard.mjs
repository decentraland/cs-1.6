...
const glyphs = new Map(
  Object.values(atlas.fonts).flatMap((font) =>
    Object.entries(font.glyphs).map(([character, rect]) => [rect.slice(0, 4).join(','), character])
  )
)
...
export function readTextEntities(state) {
  ...
  for (const value of Object.values(state)) {
    ...
    const rect = [
      Math.round(uv[0] * atlas.width),
      Math.round((1 - uv[3]) * atlas.height),
      Math.round((uv[4] - uv[0]) * atlas.width),
      Math.round((uv[3] - uv[1]) * atlas.height)
    ]
    const character = glyphs.get(rect.join(','))
    if (character === undefined) continue
    const group = groups.get(transform.parent) ?? []
    group.push({ character, left: transform.positionLeft })
    groups.set(transform.parent, group)
  }
  for (const [parent, glyphs] of groups)
    result.push({
      UiText: {
        value: glyphs
      ...
      UiTransform: state[parent].UiTransform
    })
  return result
}
...
export function readTeamScore(state, team) {
  ...
  const score = text.find(
    (value) => value.UiTransform?.parent === header.UiTransform.parent && /^\d+$/.test(value.UiText.value)
  )
...
export function readScoreRows(state) {
  ...
  for (const text of readTextEntities(state)) {
    ...
    const parent = text.UiTransform.parent,
  ...
  return [...groups]
    .map(([id, cells]) => ({
      id,
      top: state[id]?.UiTransform?.positionTop ?? 0,
      values: cells
        .sort((a, b) => a.UiTransform.positionLeft - b.UiTransform.positionLeft)
        .map((cell) => cell.UiText.value)
    }))
