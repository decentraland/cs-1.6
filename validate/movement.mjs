...
import { equippedGun, GUN_SPEEDS, isBotRoundLive, joinCounterTerrorists } from './solo-flow.mjs'

const endpoint = process.argv[2]
if (!endpoint) throw new Error('Usage: node validate/movement.mjs <browser-CDP-websocket> [evidence.json]')
...
async function startGame() {
  const frozen = await joinCounterTerrorists(io)
  ...
  const live = await stateUntil(isBotRoundLive, 'live phase')
...
try {
  ...
  if (labels(await snapshot()).includes('Select a team')) await startGame()
  else await stateUntil((s) => labels(s).includes('Prepare to fight!'), 'next freeze', 30000)
  const armed = await stateUntil(isBotRoundLive, 'next live')
  // Movement speed follows the equipped gun (221 AK, 230 M4A1, 250 pistols/knife units × 0.025); walking is 52%.
  const gun = equippedGun(armed)
  const runSpeed = GUN_SPEEDS[gun]
  assert.ok(runSpeed, `known gun speed for ${gun}`)
  const measure = async (walk) => {
    ...
    const p = before['1'].Transform.position,
      q = after['1'].Transform.position
    return {
      speed: Math.hypot(q.x - p.x, q.z - p.z) / seconds,
      settings: after['1'].AvatarMovementInfo.activeAvatarLocomotionSettings,
      velocity: after['1'].AvatarMovementInfo.actualVelocity
    }
  }
  const run = await measure(false)
  await pause(250)
  const walk = await measure(true)
  assert.ok(Math.abs(run.speed - runSpeed) < 0.85, JSON.stringify({ gun, runSpeed, run }))
  assert.ok(Math.abs(walk.speed - runSpeed * 0.52) < 0.5, JSON.stringify({ gun, runSpeed, walk }))
  assert.ok(walk.speed / run.speed < 0.65)
  console.log(JSON.stringify({ gun, runSpeed, run, walk }, null, 2))
  if (process.argv[3]) await writeFile(process.argv[3], JSON.stringify({ gun, runSpeed, run, walk }, null, 2) + '\n')
