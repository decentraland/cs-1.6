...
export function readHud(state) {
  ...
  for (const value of Object.values(state)) {
    ...
    const x = Math.round(uv[0] * 256),
      y = Math.round((1 - uv[3]) * 256)
    const width = Math.round((uv[4] - uv[0]) * 256),
      height = Math.round((uv[3] - uv[1]) * 256)
  ...
  const at = (left) => {
    const group = [...groups].find(([parent]) => Math.abs(state[parent]?.UiTransform?.positionLeft - left) < 0.1)
  ...
  const minutes = at(timerX + 24 * scale),
    seconds = at(timerX + 74 * scale)
  return {
    health: at(34 * scale),
    armor: at(width / 5 + 24 * scale),
    clip: at(width - 184 * scale),
    reserve: at(width - 102 * scale),
    money: at(width - 108 * scale),
    seconds: minutes === undefined || seconds === undefined ? undefined : minutes * 60 + seconds
  }
}
