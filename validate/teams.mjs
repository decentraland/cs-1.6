...
if (endpoints.length !== 2) throw new Error('Usage: node validate/teams.mjs <CT-CDP-websocket> <T-CDP-websocket> [evidence.json]')
...
try {
  ...
  await ct.join(2,true)
  await ct.until(s=>labels(s).includes('Joined Counter-Terrorists'),'CT admission')
  await t.until(s=>labels(s).includes('Select a team'),'shared waiting lobby')
  ...
  await ct.key('keyDown','w','KeyW',87);await pause(450);await ct.key('keyUp','w','KeyW',87)
  const after=(await ct.snapshot())['1'].Transform.position
  assert.ok(Math.hypot(after.x-evidence.spawns[0].x,after.z-evidence.spawns[0].z)<.1,'freeze blocks movement')
  await ct.until(s=>!labels(s).includes('Prepare to fight!')&&readHud(s)?.seconds>100,'live')
  evidence.checks.push('explicit teams, shared freeze, original spawns')
  const ctBoard=await ct.scoreboard(), tBoard=await t.scoreboard()
  ...
  for(const board of [ctBoard,tBoard]) {
    ...
    assert.equal(Object.values(board).filter(value=>Math.abs((value.UiBackground?.color?.a??0)-16/255)<.001).length,1,'only the local player row is highlighted')
  }
  ...
  async function encounter(shooter,victim,winner,score) {
    ...
    const victimState=await victim.snapshot(), feet=victimState['1'].Transform.position
    await shooter.aimAt({x:feet.x,y:feet.y+1.6,z:feet.z})
    for(let i=0;i<5;i++) {
      await shooter.shoot(); await pause(450)
      if(readHud(await victim.snapshot())?.health===0) break
    }
    const dead=await victim.until(s=>readHud(s)?.health===0&&labels(s).includes(`${winner} Win!`),'shared kill/result')
    const result=await shooter.scoreboard(), victimBoard=await victim.scoreboard()
    ...
    await victim.key('keyDown','w','KeyW',87);await victim.shoot(300);await pause(400);await victim.key('keyUp','w','KeyW',87)
    const stillDead=await victim.snapshot(), pos=stillDead['1'].Transform.position
    assert.equal(readHud(stillDead).health,0)
    assert.equal(readHud(stillDead).clip,readHud(dead).clip,'dead player cannot fire')
    assert.ok(Math.hypot(pos.x-before.x,pos.z-before.z)<.1,'dead movement blocked')
  ...
  evidence.ctWin=await encounter(ct,t,'Counter-Terrorists',1)
  ...
  for(const client of [ct,t])assert.equal(readHud(await client.snapshot()).health,100,'health restored at round boundary')
  await t.until(s=>!labels(s).includes('Prepare to fight!')&&readHud(s)?.seconds>100,'second live round')
  evidence.tWin=await encounter(t,ct,'Terrorists',1)
