...
try {
 ...
 const fireAnimation=state=>Object.values(state).some(value=>value.GltfContainer?.src==='assets/scene/weapons/ak47.glb'&&value.Animator?.states?.some(animation=>animation.clip==='fire'&&animation.playing))
 const samples=[]
 ...
 while(Date.now()-started<900) {
  const state=await snapshot()
  samples.push({ms:Date.now()-started,pitch:angles(state['2'].Transform.rotation).pitch-base.pitch,fireAnimation:fireAnimation(state),impact:spheres(state).some(id=>!initialSpheres.has(id))})
  await pause(12)
 }
 const kick=samples.find(sample=>sample.pitch>0.003)
 const impact=samples.find(sample=>sample.impact)
 const animation=samples.find(sample=>sample.fireAnimation)
 assert.ok(kick&&impact&&animation,'camera kick, weapon fire animation, and server impact were observed')
 assert.ok(kick.ms<200,`camera kick appeared after ${kick.ms}ms`)
 assert.ok(animation.ms<200,`weapon animation appeared after ${animation.ms}ms`)
 assert.ok(impact.ms-kick.ms>150,'local camera kick precedes deliberately delayed server feedback')
 assert.ok(samples.filter(sample=>sample.ms>350).every(sample=>Math.abs(sample.pitch)<0.003),'late acknowledgement does not kick the camera again')
 console.log('FEEDBACK',JSON.stringify({kick:kick.ms,animation:animation.ms,impact:impact.ms}))
 const gap=state=>{
  const arms=Object.values(state).filter(value=>value.UiTransform?.width===10&&value.UiTransform?.height===2&&value.UiBackground?.color?.g===1)
 ...
 await send('Input.dispatchKeyEvent',{type:'keyDown',key:'Shift',code:'ShiftLeft',windowsVirtualKeyCode:16,modifiers:8})
 await send('Input.dispatchKeyEvent',{type:'keyDown',key:'w',code:'KeyW',windowsVirtualKeyCode:87,modifiers:8})
 await record('walk-forward',500)
 await send('Input.dispatchKeyEvent',{type:'keyDown',key:'d',code:'KeyD',windowsVirtualKeyCode:68,modifiers:8})
 await record('walk-diagonal',700)
 await send('Input.dispatchKeyEvent',{type:'keyUp',key:'d',code:'KeyD',windowsVirtualKeyCode:68,modifiers:8})
 await record('walk-forward-again',400)
 await send('Input.dispatchKeyEvent',{type:'keyUp',key:'w',code:'KeyW',windowsVirtualKeyCode:87,modifiers:8})
 await send('Input.dispatchKeyEvent',{type:'keyUp',key:'Shift',code:'ShiftLeft',windowsVirtualKeyCode:16,modifiers:0})
 assert.ok(motion.every(sample=>sample.gap<=6),`walking crosshair gaps: ${motion.map(sample=>sample.gap)}`)
 const diagonal=motion.filter(sample=>sample.label==='walk-diagonal')
 const first=diagonal[0],last=diagonal.at(-1)
 const speed=Math.hypot(last.position.x-first.position.x,last.position.z-first.position.z)/((last.ms-first.ms)/1000)
 ...
 const evidence={date:new Date().toISOString(),feedbackDelayMs:400,firstKickMs:kick.ms,firstAnimationMs:animation.ms,firstImpactMs:impact.ms,diagonalSpeed:speed,runningGap:gap(run),samples,motion}
 if(process.argv[3])await writeFile(process.argv[3],JSON.stringify(evidence,null,2)+'\n')
 console.log('PASS: immediate camera/weapon feedback → delayed acknowledgement without second kick → stable Shift W+D crosshair → running expansion')
 console.log(JSON.stringify({firstKickMs:kick.ms,firstAnimationMs:animation.ms,firstImpactMs:impact.ms,diagonalSpeed:speed}))
} finally {
 if(sessionId) {
  for(const [key,code,vk]of[['w','KeyW',87],['d','KeyD',68],['s','KeyS',83],['Shift','ShiftLeft',16]])await send('Input.dispatchKeyEvent',{type:'keyUp',key,code,windowsVirtualKeyCode:vk,modifiers:0})
