...
import { isBotRoundLive, readViewmodel } from './solo-flow.mjs'
...
const viewmodelRotation = (state) => readViewmodel(state)?.rotation ?? { x: 0, y: 0, z: 0, w: 1 }
...
const botAvatars = (state) =>
  Object.values(state).filter((component) => component.AvatarShape?.name?.startsWith('BOT ') && component.Transform)
const painSprites = (state) =>
  Object.values(state).filter(
    (component) => component.UiBackground?.texture?.tex?.texture?.src === 'assets/ui/pain.png'
  )
const damageSound = (state) =>
  Object.values(state).find((component) => component.AudioSource?.audioClipUrl === 'assets/sounds/player/damage.wav')
const rotationDistance = (left, right) =>
  2 * Math.acos(Math.min(1, Math.abs(left.x * right.x + left.y * right.y + left.z * right.z + left.w * right.w)))
const cameraViewMeshes = (state) => {
  ...
  return entries
    .filter(([id, component]) => descendants.has(id) && (component.MeshRenderer || component.GltfContainer))
    .map(([, component]) => component)
}

try {
  await client.connect()
  await client.command('/set_scene Counter-Strike')
  await client.until((state) => labels(state).includes('Select a team'), 'fresh team menu', 30000)
  await client.join(2)
  const live = await client.until(isBotRoundLive, 'live round against T bots')
  await client.capture()
  const firstBot = botAvatars(live)[0]
  assert.ok(firstBot, 'live bot round renders a bot avatar')
  const staging = {
    x: firstBot.Transform.position.x + 3,
    y: firstBot.Transform.position.y,
    z: firstBot.Transform.position.z
  }
  await client.command(`/move_player_to ${staging.x} ${staging.y} ${staging.z}`)
  const exposed = await client.until(
    (state) => distance(player(state), staging) < 0.5,
    'player reaches bot firing lane'
  )
  const hit = await client.until(
    (state) => {
    ...
    'bot hit produces a directional pain indicator and impact sound',
    10000,
    0
  )
  assert.ok(
    painSprites(hit).some((sprite) => sprite.UiTransform?.width === 128 && sprite.UiTransform?.height === 48),
    'front damage uses the original 128×48 pain frame'
  )
  // The pain punch tilts the first-person viewmodel; the engine camera is never rotated.
  assert.ok(
    rotationDistance(viewmodelRotation(exposed), viewmodelRotation(hit)) > 0.001,
    'damage hit jolts the weapon viewmodel immediately'
  )
  if (evidencePath) {
  ...
  const chase = await client.until(
    (state) => {
      if (!spectatorLabel(state)) return false
      return botAvatars(state).some(
        (bot) => distance(camera(state), { ...bot.Transform.position, y: bot.Transform.position.y + 1.6 }) < 3.2
      )
    },
    'camera chases a living bot after death',
    4500
  )
      ...
      JSON.stringify(
        {
          ...
          fixture: extendedHitWindow
            ? 'Isolated bot fire interval extended to 500 ms for frame-level pain sprite capture'
            : undefined,
          handoffMs,
          corpse,
          initialCamera,
          ...
          hitViewmodelRadians: rotationDistance(viewmodelRotation(exposed), viewmodelRotation(hit)),
          checks: [
            'directional CS pain sprite',
            'impact sound',
            'hitgroup and armor-dependent viewmodel punch',
            'camera fall and 80-degree roll rules',
