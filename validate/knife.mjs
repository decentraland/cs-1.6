...
if (process.argv.length < 4)
  throw new Error('Usage: node validate/knife.mjs <CT-CDP-websocket> <T-CDP-websocket> [evidence.json] (fresh realm)')
const [ct, t] = process.argv.slice(2, 4).map((endpoint) => new Client(endpoint))
const evidence = { date: new Date().toISOString(), checks: [] }
async function tap(client, key, code, vk) {
  await client.key('keyDown', key, code, vk)
  await pause(80)
  await client.key('keyUp', key, code, vk)
  await pause(250)
}
async function selectKnife(client) {
  await tap(client, '3', 'Digit3', 51)
  await client.until((state) => labels(state).some((value) => value.startsWith('Knife   1:')), 'knife selected')
}
const humanScore = (state, score, deaths) =>
...
try {
  ...
  const selected = await ct.snapshot(),
    knifeParts = Object.values(selected).filter(
      (value) => value.MeshRenderer?.mesh?.box && value.VisibilityComponent?.visible
    )
  assert.equal(readHud(selected).clip, undefined)
  assert.equal(readHud(selected).reserve, undefined)
  assert.ok(knifeParts.length >= 3, 'knife proxy is visible while gun proxies are hidden')
  evidence.checks.push('3 equips the visible knife and hides firearm ammunition')
  ...
  await t.aimAt({ x: ctFeet.x, y: ctFeet.y + 1.05, z: ctFeet.z })
  await pause(550)
  const tFeet = (await t.snapshot())['1'].Transform.position
  await ct.aimAt({ x: tFeet.x, y: tFeet.y + 1.05, z: tFeet.z })
  await tap(ct, 'f', 'KeyF', 70)
  await t.until((state) => readHud(state)?.health === 0, '65-damage front stab finishes target')
  assert.ok(
    labels(await ct.snapshot()).some((value) => value.includes('Knife')),
    'knife kill feed'
  )
  ...
  await selectKnife(ct)
  await t.command('/move_player_to 90 11 53')
  await ct.command('/move_player_to 90 11 52.3')
  await pause(1200)
  await t.capture()
  await t.aimAt({ x: 90, y: 12.6, z: 63 })
  await pause(500)
  const victim = (await t.snapshot())['1'].Transform.position
  await ct.capture()
  await ct.aimAt({ x: victim.x, y: victim.y + 1.05, z: victim.z })
  await tap(ct, 'f', 'KeyF', 70)
  await t.until((state) => readHud(state)?.health === 0, 'one-hit backstab')
  const board = await ct.scoreboard()
  assert.ok(humanScore(board, 2, warmUp.firstDeaths), 'both knife kills reach the CT scoreboard row')
