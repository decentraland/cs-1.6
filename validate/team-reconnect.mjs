...
function scoreRows(s) {
 ...
 return [...groups.values()].filter(row=>row.length===4&&row.some(c=>c.value.startsWith('0x'))).map(row=>{
  const values=[];let previous=0
  while(values.length<row.length){const c=row.find(c=>c.previous===previous);assert.ok(c,'scoreboard row order');values.push(c.value);previous=c.id}
  return values
 })
}
...
try {
 ...
 await returning.key('keyDown','1','Digit1',49);await pause(250)
 const before=await returning.snapshot();evidence.before=scoreRows(before)
 await returning.key('keyUp','1','Digit1',49)
 await returning.send('Page.setWebLifecycleState',{state:'frozen'});suspended=true
 await pause(23000)
 await returning.send('Page.setWebLifecycleState',{state:'active'});suspended=false
 ...
 await returning.key('keyDown','1','Digit1',49);await pause(200)
 evidence.after=scoreRows(await returning.snapshot())
 await returning.key('keyUp','1','Digit1',49)
 ...
 assert.deepEqual(evidence.after.map(row=>row.find(v=>v.startsWith('0x'))).sort(),names,'same player addresses remain on scoreboard')
 ...
 await ct.command('/move_player_to 95 10.1 52');await other.command('/move_player_to 90 10.1 52');await pause(2000)
 await ct.capture();const feet=(await other.snapshot())['1'].Transform.position
 await ct.aimAt({x:feet.x,y:feet.y+1.6,z:feet.z})
 for(let i=0;i<5;i++){await ct.shoot();await pause(450);if(readHud(await other.snapshot()).health===0)break}
 await returning.until(s=>labels(s).includes('Counter-Terrorists Win!'),'queued returning player cannot prevent elimination')
 await returning.until(s=>labels(s).includes('Prepare to fight!')&&readHud(s)?.health===100,'same player respawns next round',8000)
 evidence.checks=['same-session heartbeat expiry','same addresses retained','rejoin stays dead','next-round spawn']
