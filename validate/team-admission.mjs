...
try {
 ...
 await ct.until(s=>readHud(s)?.seconds>50&&!labels(s).includes('Prepare to fight!'),'live round with time for disconnect',130000)
 ...
 await t.send('Target.closeTarget',{targetId:target.targetId});t.sessionId=undefined
 await ct.until(s=>labels(s).includes('Counter-Terrorists Win!'),'disconnected final opponent ends round',30000)
 ...
 await late.until(s=>labels(s).includes('Prepare to fight!')&&readHud(s)?.health===100,'queued player spawns at next round',8000)
 ...
 while (Date.now() < readyDeadline) {
   try { ready = await t.evaluate("typeof window.engine_console_command === 'function'") } catch (error) {
 ...
 await pause(2000);await ct.capture()
 const feet=(await late.snapshot())['1'].Transform.position
 await ct.aimAt({x:feet.x,y:feet.y+1.6,z:feet.z})
 for(let i=0;i<5;i++) { await ct.shoot();await pause(450);if(readHud(await late.snapshot()).health===0)break }
 await t.until(s=>labels(s).includes('Counter-Terrorists Win!'),'queued reconnect observes result')
 await t.until(s=>labels(s).includes('Prepare to fight!')&&readHud(s)?.health===100,'reconnected player spawns with next round',8000)
