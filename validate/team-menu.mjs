...
import { autoAssignButtonPoint, practiceButtonPoint } from './menu-layout.mjs'
...
const closeTo = (actual, expected, description) => assert.ok(Math.abs(actual - expected) <= 1, `${description}: ${actual} ≈ ${expected}`)
const texture = value => value.UiBackground?.texture?.tex?.texture?.src
const gunLabel = (state, name) => labels(state).some(value => value.startsWith(`${name}   2:`))
...
function assertMenu(state, width, height) {
  ...
  const expected = [
    'SELECT TEAM', '1 TERRORIST FORCES', '2 CT FORCES', '5 AUTO ASSIGN', '6 SPECTATE',
    'PRACTICE WITH BOTS', 'Dust II - Bomb/Defuse', '*** GameHelper.com exclusive ***',
    'Counter-Terrorists: Prevent Terrorists', 'from bombing chemical weapon crates.',
    'Terrorists: The Terrorist carrying the', 'C4 must destroy one of the chemical',
  ...
  const transforms = Object.values(state).map(value => value.UiTransform).filter(Boolean)
  const outer = transforms.find(value => Math.abs(value.width - 600 * coordinates.scale) <= 1 && Math.abs(value.height - 440 * coordinates.scale) <= 1)
  ...
  const map = transforms.find(value => Math.abs(value.width - 316 * coordinates.scale) <= 1 && Math.abs(value.height - 286 * coordinates.scale) <= 1)
  ...
  const buttons = transforms.filter(value => Math.abs(value.width - 148 * coordinates.scale) <= 1 && Math.abs(value.height - 20 * coordinates.scale) <= 1 && value.borderLeftWidth === 1)
  assert.equal(buttons.length, 5, 'four source team controls plus the practice extension')
  const tops = buttons.map(value => value.positionTop).sort((a, b) => a - b)
  for (const [index, sourceTop] of [116, 148, 212, 244, 308].entries()) closeTo(tops[index], coordinates.y(sourceTop), `button ${index + 1} top`)
  ...
  return { width, height, scale: coordinates.scale, outer: { left: outer.positionLeft, top: outer.positionTop, width: outer.width, height: outer.height } }
}
...
async function setViewport(client, width, height) {
  ...
  return client.until(state => {
    ...
    return Object.values(state).some(value => Math.abs((value.UiTransform?.width ?? 0) - expectedWidth) <= 1 && Math.abs((value.UiTransform?.height ?? 0) - 440 * Math.min(width / 640, height / 480)) <= 1)
...
if (mode === 'visual') {
  ...
  try {
    ...
    await client.until(state => labels(state).includes('Select a team') && labels(state).includes('Start game'), 'fresh team menu', 45000)
    ...
    const point = practiceButtonPoint(1280, 720)
    await client.click(point.x, point.y)
    await client.until(state => labels(state).includes('Prepare to fight!'), 'practice button starts freeze time')
    await writeFile(join(evidenceDir, 'visual.json'), JSON.stringify({
      date: new Date().toISOString(), compactLayout, wideLayout,
      checks: ['exact visible team labels and Dust II briefing', 'resource coordinates at 800×450 and 1280×720', 'original logo', 'practice button starts freeze time']
    }, null, 2) + '\n')
    console.log('PASS: CS team menu text, source geometry, responsive framing, logo, and practice action')
  } finally {
    if (client.sessionId) await client.send('Emulation.clearDeviceMetricsOverride').catch(error => console.warn(error.message))
    client.socket.close()
  }
} else {
  ...
  try {
    ...
    const active = await Promise.all([
      a.until(state => labels(state).includes('Prepare to fight!') && gunLabel(state, 'Glock-18'), 'Terrorist receives Glock in freeze time'),
      b.until(state => labels(state).includes('Prepare to fight!') && gunLabel(state, 'USP'), 'Counter-Terrorist receives USP in freeze time')
    ])
    await writeFile(join(evidenceDir, 'auto-assign.json'), JSON.stringify({
      date: new Date().toISOString(), firstAssignment: 'Terrorists', secondAssignment: 'Counter-Terrorists',
      firstLoadout: gunLabel(active[0], 'Glock-18') ? 'Glock-18' : undefined,
      secondLoadout: gunLabel(active[1], 'USP') ? 'USP' : undefined,
      checks: ['first player is assigned to Terrorists on a tie', 'second player is assigned to Counter-Terrorists', 'balanced teams start freeze time']
    }, null, 2) + '\n')
    console.log('PASS: auto-assign balances T then CT and starts a shared round')
  } finally {
