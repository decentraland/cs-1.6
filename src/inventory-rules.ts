...
export interface StoredGun { id: string; clip: number; reserve: number }
export interface GunInventory { active: string; items: StoredGun[] }
export function startingInventory(team: number): GunInventory {
  const gun=team===1?GUNS.glock18:GUNS.usp
  return {active:gun.id,items:[{id:gun.id,clip:gun.clip,reserve:gun.clip*2}]}
}
export function buyGun(inventory: GunInventory, id: string, team: number, money: number): {money:number;error?:string} {
  ...
  inventory.items.push({id,clip:gun.clip,reserve:0});inventory.active=id
...
export function buyGunAmmo(inventory: GunInventory, slot: string, money: number): {money:number;error?:string} {
  const item=inventory.items.find(item=>gunProfile(item.id)?.slot===slot),gun=item&&gunProfile(item.id)
...
export function selectWeapon(inventory: GunInventory, slot: string): WeaponId | undefined {
  if(slot==='melee') { inventory.active='knife';return 'knife' }
...
export class SemiAutoTrigger {
  ...
  private spent=0
  private held=false
  update(held:boolean) { if(held&&!this.held)this.press++;this.held=held }
  claim() { if(this.press<=this.spent)return false;this.spent=this.press;return true }
  reset() { this.press=0;this.spent=0;this.held=false }
}
