...
import { Dead, PlayerAddress, PlayerEquipment, PlayerHealth, PlayerMoney, PlayerTeam, PlayerInventory, Weapon } from './components'
...
import { creditedMoney, freshLossHistory, purchaseEquipment, roundPayments, START_MONEY, PurchaseSequences, playerRoundPayment } from './economy-rules'
...
export function resetEconomy() {
  losses = freshLossHistory(); pending.clear(); purchases.clear()
...
export function settleRoundEconomy(winner: 'ct' | 't' | 'draw', bombPhase: string, timeout: boolean) {
  ...
  if (!match || match.mode !== 'teams') return
  ...
  for (const seat of match.roster) {
    ...
    pending.set(seat.address,playerRoundPayment(seat.team === 1 ? amounts.t : amounts.ct,seat.team,!Dead.has(player),timeout))
  }
}
...
export function initializeEconomy() {
  room.onMessage('buyEquipment',(data,context)=>{
    ...
    if (player === undefined || !match || match.mode !== 'teams' || data.round !== match.round) return
    if (!purchases.accept(address,data.sequence)) return
    const health = PlayerHealth.getMutable(player), equipment = PlayerEquipment.getMutable(player), money = PlayerMoney.getMutable(player), weapon = Weapon.getMutable(player)
    const team = PlayerTeam.get(player).team, feet = playerPosition(address)
    const buyContext = {alive:!Dead.has(player)&&health.current>0,eligible:canPlayRound(address),team,phase:match.phase,elapsed:roundElapsed(),inZone:!!feet&&inBuyZone(feet,team)}
    if(data.item.startsWith('weapon:') || data.item==='ammo' || data.item==='secondaryammo') {
      const restriction=buyRestriction(buyContext)
      if(restriction){room.send('matchNotice',{address,message:restriction});return}
      storeActiveGun(player)
      const inventory=PlayerInventory.getMutable(player)
      const result=data.item.startsWith('weapon:')?buyGun(inventory,data.item.slice(7),team,money.amount):buyGunAmmo(inventory,data.item==='ammo'?'primary':'secondary',money.amount)
      if(result.error){room.send('matchNotice',{address,message:result.error});return}
      money.amount=result.money
      if(data.item.startsWith('weapon:'))equipActiveWeapon(player)
      else { const active=inventory.items.find(item=>item.id===inventory.active);if(active)weapon.ammoReserve=active.reserve }
      return
    }
    ...
    money.amount = account.money; health.armor = account.armor
    equipment.helmet = account.helmet; equipment.defuseKit = account.defuseKit; weapon.ammoReserve = account.reserve
  })
}
