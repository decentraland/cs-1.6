...
export const freshAccuracy = (): AccuracyState => ({
  shots: 0, accuracy: 0.2, lastFire: 0, held: false, recoverAt: 0,
  updatedAt: 0, pitch: 0, yaw: 0, right: false
})
...
export function recoverAccuracy(state: AccuracyState, now: number) {
  ...
  if (original > 0) { state.pitch *= length / original; state.yaw *= length / original }
  state.updatedAt = now
...
export function setTrigger(state: AccuracyState, held: boolean, now: number) {
  ...
  state.held = held
  if (!held) { state.shots = Math.min(15, state.shots); state.recoverAt = now + 0.4 }
}

export function akSpread(state: AccuracyState, now: number, speed: number, grounded: boolean): number {
  recoverAccuracy(state, now)
  const spread = !grounded ? 0.04 + 0.4 * state.accuracy
    : speed > 140 * 0.025 ? 0.04 + 0.07 * state.accuracy : 0.0275 * state.accuracy
  state.shots++
...
export function kickBack(state: AccuracyState, speed: number, grounded: boolean, crouched: boolean, random = Math.random) {
  const [up, side, upStep, sideStep, maxUp, maxSide, change] = speed > 0.001
    ? [1.5, 0.45, 0.225, 0.05, 6.5, 2.5, 7]
    : !grounded ? [2, 1, 0.5, 0.35, 9, 6, 5]
    : crouched ? [0.9, 0.35, 0.15, 0.025, 5.5, 1.5, 9]
