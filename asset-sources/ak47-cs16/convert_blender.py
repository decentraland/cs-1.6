...
def import_weapon(source, output):
    ...
    for part in model.meshes:
        ...
        mesh.from_pydata(vertices, [], [[corner[0] for corner in face] for face in part['faces']])
        mesh.update()
        ...
        normals = []
        for poly, face, material_id in zip(mesh.polygons, part['faces'], part['materials']):
    ...
    report = {'name': name, 'sourceSha256': scene['source_sha256'], 'triangles': sum(len(p['faces']) for p in model.meshes),
