"""Import stock player bodies and neutral-blend animations through Blender MCP."""

from pathlib import Path
import hashlib
import json
from prepare_player import prepare_player, included
import math
import struct
import zlib
...
from player_model import Model

UNIT = 0.025
AXIS = Matrix.Translation((0, 0, 36 * UNIT)) @ Matrix.Rotation(-math.pi / 2, 4, 'Z')
...
def import_player(source, output):
    source, output = Path(source), Path(output)
    output.mkdir(parents=True, exist_ok=True)
    model = Model(source)
    unique = {}
    for sequence in model.sequences:
        if included(sequence['name']):
            unique.setdefault(sequence['name'], sequence)
    model.sequences = list(unique.values())
    model.meshes = [part for part in model.meshes if part['faces']]
    name = source.stem
    previous_scene = bpy.context.window.scene
    if bpy.context.object and bpy.context.object.mode != 'OBJECT':
        raise RuntimeError('Finish the active edit operation before importing weapons')
    scene = bpy.data.scenes.new('CS16 player ' + name)
    ...
    # Bind against Blender's stored rest matrices, which can round edit-bone roll.
    world_rest = [armature.bones[bone['name']].matrix_local.copy() for bone in model.bones]
    local_rest = [world_rest[bone['parent']].inverted() @ world_rest[i] if bone['parent'] >= 0 else world_rest[i]
                  for i, bone in enumerate(model.bones)]
    objects = []
    skipped_degenerate = 0
    for part in model.meshes:
        mesh = bpy.data.meshes.new(name + ' ' + part['name'])
        # Opposite faces can share source vertices but have different UVs (Urban's collar).
        ...
        corners = list(dict.fromkeys(corner for face, _ in faces for corner in face))
        remap = {corner: i for i, corner in enumerate(corners)}
        vertices = [world_rest[part['weights'][c[0]]] @ (Vector(part['points'][c[0]]) * UNIT) for c in corners]
        mesh.from_pydata(vertices, [], [[remap[corner] for corner in face] for face, _ in faces])
        mesh.update()
        ...
        for i, bone in enumerate(model.bones):
            indices = [j for j, corner in enumerate(corners) if part['weights'][corner[0]] == i]
            if indices:
    ...
    for sequence in model.sequences:
        ...
        for index, bone in enumerate(model.bones):
            ...
            for frame in range(sequence['frames']):
                ...
                location, rotation, _scale = (inverse @ matrix).decompose()
                ...
                for channel, value in zip(channels, [*location, *rotation]):
                    channel.extend([at, value])
            pose_bone.keyframe_insert(data_path='location', frame=0, group=bone['name'])
            pose_bone.keyframe_insert(data_path='rotation_quaternion', frame=0, group=bone['name'])
            slot = rig.animation_data.action_slot
            bag = action.layers[0].strips[0].channelbag(slot)
            for path, count, offset in [('location',3,0),('rotation_quaternion',4,3)]:
                for i in range(count):
                    curve = bag.fcurves.find(pose_bone.path_from_id(path), index=i)
                    curve.keyframe_points.add(sequence['frames'] - 1)
                    curve.keyframe_points.foreach_set('co', channels[offset+i])
    ...
    runtime = prepare_player(path, model)
    rig.animation_data.action, rig.animation_data.action_slot = actions[idle]
    scene.frame_set(0)
    bpy.context.view_layer.update()
    bpy.data.libraries.write(str(output / (name + '.blend')), {scene}, fake_user=True)
    report = {'name': name, 'scene':scene.name, 'sourceSha256': scene['source_sha256'], 'triangles': sum(len(p['faces']) for p in model.meshes) - skipped_degenerate,
              ...
              'runtime': runtime, 'bytes': path.stat().st_size, 'blender': bpy.app.version_string}
