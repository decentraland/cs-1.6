"""Read stock player MDLs with explicit bodygroup and neutral blend selection."""
...
class Model:
    def __init__(self, path):
        ...
        for i in range(count):
            ...
            assert models == 1 or self.string(at, 64) == 'backpack', 'Unknown bodygroup'
            # Bodygroup zero is the standard player without the C4 backpack.
            at = model_offset
        ...
        for i in range(count):
            ...
            assert group == 0 and blends in (1, 9), 'Unknown player blend format'
            animation += (4 if blends == 9 else 0) * len(self.bones) * 12
            channels = []
            ...
            self.sequences.append(dict(name=self.string(at, 32), fps=fps, frames=frames, flags=flags, channels=channels,
                activity=self.unpack('i', at+40)[0], weight=self.unpack('i', at+44)[0],
                motion=self.unpack('3f', at+76), sourceIndex=i, blend=4 if blends==9 else 0))
